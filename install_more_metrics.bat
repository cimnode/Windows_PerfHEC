...
IF NOT EXIST "%cd%\moremetrics2hec.ps1" (
    ECHO %cd%\moremetrics2hec.ps1 not found. This script must be run from directory containing this file. Exiting.
    GOTO:EOF
) 
...
powershell -Command Unblock-File -Path '%cd%\moremetrics2hec.ps1'

ECHO Creating scheduled task.
SCHTASKS /CREATE /RU SYSTEM /SC minute /MO 1 /TN "Splunk More Metrics2HEC" /TR "powershell -File '%cd%\moremetrics2hec.ps1' https://%SPLUNK_SERVER%:8088/services/collector %SPLUNK_HEC_TOKEN% %SPLUNK_INDEX%"
